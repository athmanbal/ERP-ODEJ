<table id="TableGrades" class="w-full border-collapse bg-white text-left text-sm text-gray-500">
    ...
    <tbody class="divide-y divide-gray-100 border-t border-gray-100 bg-gray-10">
        @foreach ($grades as $grade)
            <tr class="hover:bg-gray-200">
                <td class="px-6 py-4 font-bold text-gray-900">{{ $grade->code_grade }}</td>
                <td class="px-6 py-4">{{ $grade->nom_grade }}</td>
                <td class="px-6 py-4">{{ $grade->bonification }}</td>
                <td class="px-1 py-1">
                    <div class="flex justify-end gap-4">
                        <a href="{{ route('grades.edit', $grade->id_grade) }}"
                        ...
                        <form action="{{ route('grades.destroy', $grade->id_grade) }}"
                            method="POST" onsubmit="return openCustomConfirmGrade(event, this);">
                            @csrf
                            @method('DELETE')
                            <button type="submit"
